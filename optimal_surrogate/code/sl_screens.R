...
# We'll need to think about what the correct functions for case.weights and scale_pos_weight are -- these depend on the weights we want to use for class balancing, and we haven't specified them yet. I assume we might want something as simple as 
# scale_pos_weight = sum(1 - Y) / sum(Y); this is the recommended choice
# and 
# case.weights = Y * sum(Y) ^ (-1) + (1 - Y) * sum(1 - Y) ^ (-1); this ensures that cases have higher weight than controls;
#############################################################################################
#############################################################################################
...
if (run_prod) {
  ...
  methods1 <- c("SL.mean", "SL.glm", "SL.glmnet", 
