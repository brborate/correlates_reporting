...
tab <- cvaucs_d57_vacc %>%
  ...
  mutate(Screen = fct_relevel(Screen, c("all", "glmnet", "univar_logistic_pval",
         ...
         Learner = fct_relevel(Learner, c("SL.mean", "SL.glmnet", "SL.xgboost.2.no", "SL.xgboost.4.no",
...
top_learner_plot <- ggplot() +
  ...
  coord_flip() +
  scale_y_continuous(breaks = seq(lowestXTick, highestXTick, 0.1), labels = seq(lowestXTick, highestXTick, 0.1), limits = c(lowestXTick, highestXTick)) +
  theme_bw() +
...
for(i in 1:length(unique(cvaucs_d57_vacc$varset))) {
  ...
  p1 <- plot_roc_curves(pred, cvaucDAT = top2, weights = ph2_vacc_ptids$wt.D57)
  print(p1)
  dev.off()
